...
class DSAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        # De-stationary Attention: rescaling pre-softmax scores with learned de-stationary factors
...
class TwoStageAttentionLayer(nn.Module):
    ...
    def __init__(self, configs, seg_num, factor, d_model, n_heads, d_ff=None, dropout=0.1):
        ...
        self.time_attention = AttentionLayer(FullAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=False), d_model, n_heads)
        self.dim_sender = AttentionLayer(FullAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=False), d_model, n_heads)
        self.dim_receiver = AttentionLayer(FullAttention(False, configs.factor, attention_dropout=configs.dropout, output_attention=False), d_model, n_heads)
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        # Cross Time Stage: Apply Multi-Head Self-Attention to each dimension
        batch = x.shape[0]
        ...
        # Cross Dimension Stage: Aggregate and distribute messages to build D-to-D connections
